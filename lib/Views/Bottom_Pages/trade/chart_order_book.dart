...
class OBLevelKChart {
  ...
  const OBLevelKChart({required this.price, required this.qty});
}
...
class OrderBookTheme {
  ...
  const OrderBookTheme({
    ...
    this.bidText = const Color(0xFF2EBD85),
    this.askText = const Color(0xFFF6465D),
    this.bidBar = const Color(0x402EBD85),
    this.askBar = const Color(0x40F6465D),
...
class KChartOrderBook extends StatelessWidget {
  ...
  final int qtyDigits;
  final void Function(String side, OBLevelKChart level, int index)? onSelect;
  ...
  List<OBLevelKChart> _top(List<OBLevelKChart> list) => list.take(rows).toList();

  List<OBLevelKChart> _cumulate(List<OBLevelKChart> levels) {
    double acc = 0;
    return levels.map((l) {
      acc += l.qty;
      return OBLevelKChart(price: l.price, qty: acc);
    }).toList();
  }
                  ...
                  // if (controller.orderBookType == 0 || controller.orderBookType == 2)
                    Expanded(
                      child: _OrderBookSection(
                        ...
                        type: type,
                        sideVal: side,
                        coinOne: controller.selectedCoinOne,
                        coinTwo: controller.selectedCoinTwo,
                        reverse: false,
                        headerShow: true,
                      ),
                    ),
      ...
      child: Column(
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              CustomText(label: "($coinTwo)", fontSize: 10.px),
              CustomText(label: "($coinOne)", fontSize: 10.px),
            ],
          ),
        ],
      ),
...
class _OrderBookSection extends StatelessWidget {
  final String side;
  final List<OBLevelKChart> levels;
  final double maxQty;
  final OrderBookTheme theme;
  final int priceDigits;
  final int qtyDigits;
  final void Function(String, OBLevelKChart, int)? onSelect;
  final String type;
          ...
          child: ListView.builder(
            ...
            itemBuilder: (_, i) => _OrderRow(
              ...
              flashColor: side == 'bid'
                  ...
                  ? const Color(0x332EBD85)
                  : const Color(0x33F6465D),
              onPress: onSelect,
              type: type,
              sideVal: sideVal,
            ),
...
class _OrderRow extends StatefulWidget {
  ...
  final String type;
  final String sideVal;
  final void Function(String, OBLevelKChart, int)? onPress;
...
class _OrderRowState extends State<_OrderRow>
  ...
  void _onTap() {
    final controller = context.read<SpotTradeController>();

    controller.setBuySellTabIndex(value: widget.side == 'ask' ? 0 : 1);
    ...
    widget.onPress?.call(
      widget.side,
      OBLevelKChart(price: widget.price, qty: widget.qty),
      widget.i,
    );
  }
}
...
class _BuySellPercentRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final buyPercent = total == 0 ? 0 : (totalBid / total);
    final sellPercent = total == 0 ? 0 : (totalAsk / total);
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              CustomText(
                label: 'B ${(buyPercent * 100).toStringAsFixed(0)}%',
                    ...
                    child: Stack(
                      children: [
                        ...
                        AnimatedFractionallySizedBox(
                          ...
                          widthFactor: buyPercent.toDouble().clamp(0.0, 1.0),
                          child: Container(color: theme.bidText),
                        ),
                      ],
                    ),
                  ),
              ...
              CustomText(
                label: 'S ${(sellPercent * 100).toStringAsFixed(0)}%',
