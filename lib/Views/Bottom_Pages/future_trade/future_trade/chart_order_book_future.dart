...
class OBLevelKChartFuture {
  final double price;
  final double qty;

  const OBLevelKChartFuture({required this.price, required this.qty});
}
...
class KChartOrderBookFuture extends StatelessWidget {
  ...
  final int qtyDigits;
  final void Function(String side, OBLevelKChartFuture level, int index)? onSelect;
  ...
  List<OBLevelKChartFuture> _top(List<OBLevelKChartFuture> list) => list.take(rows).toList();

  List<OBLevelKChartFuture> _cumulate(List<OBLevelKChartFuture> levels) {
    double acc = 0;
    return levels.map((l) {
      acc += l.qty;
      return OBLevelKChartFuture(price: l.price, qty: acc);
    }).toList();
  }
...
class _OrderBookSection extends StatelessWidget {
  final String side;
  final List<OBLevelKChartFuture> levels;
  final double maxQty;
  final OrderBookTheme theme;
  final int priceDigits;
  final int qtyDigits;
  final void Function(String, OBLevelKChartFuture, int)? onSelect;
  final String type;
...
class _OrderRow extends StatefulWidget {
  ...
  final String type;
  final String sideVal;
  final void Function(String, OBLevelKChartFuture, int)? onPress;
